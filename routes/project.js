...
router.post("/:projectId/upload/:itemId", function (req, res) {
    req.uploadTarget = null;
    uploadFile(req, res);
});
...
router.get("/:projectId/items", function (req, res) {
    var id = req.params.projectId;
    thread.invoke(id, {command: "getItems"}).then(function (data) {
        res.json(data.payload.data);
    }, function (err) {
        handleError(err, res, threadError);
    });
